...
func TestWalk2(t *testing.T) {
	cases := []struct {
		...
		{
			"Maps",
			map[string]string{"Country": "Japan", "Capital": "Tokyo"},
			[]string{"Japan", "Tokyo"},
		},
	}
...
func TestChannels(t *testing.T) {
	aChannel := make(chan ProfileItem)
	go func() {
		aChannel <- ProfileItem{33, "Berlin"}
		aChannel <- ProfileItem{32, "Tokyo"}
		close(aChannel)
	}()
...
func TestFunctions(t *testing.T) {
	aFunction := func() (ProfileItem, ProfileItem) {
		return ProfileItem{33, "Berlin"}, ProfileItem{34, "Nairobi"}
	}

	var got []string
	want := []string{"Berlin", "Nairobi"}
